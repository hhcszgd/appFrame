...
public class GDRefreshControl: GDBaseControl {
    public var refreshHeight : CGFloat = 35
    ...
    public var pullingStr = "refresh_pullingStr"|?|
    public var loosenStr = "refresh_loosenStr"|?|
    public var refreshingStr = "refresh_refreshingStr"|?|
    public var successStr = "refresh_successStr"|?|
    public var failureStr = "refresh_failureStr"|?|
    public var networkErrorStr = "refresh_networkErrorStr"|?|
...
//                self.titleLabel.sizeToFit()
                self.titleLabel.frame = CGRect(x:0 , y: bounds.size.height * 0.5 - titleLabel.font.lineHeight/2, width: self.bounds.size.width, height: titleLabel.font.lineHeight)
            }else{
                self.titleLabel.frame = self.labelFrame
            }
            if self.imageViewFrame == CGRect.zero {
                let imgH = self.bounds.size.height * 0.6
//                let labelW = titleLabel.text?.sizeSingleLine(font: titleLabel.font).width ?? 0
                let labelW = "refresh_pullingStr"|?|.sizeSingleLine(font: titleLabel.font).width ?? 0
                let imgX : CGFloat = labelW > 0 ? self.bounds.width/2 - labelW/2 - imgH : self.bounds.width/2 - imgH * 3
                self.imageView.frame = CGRect(x: imgX - 5, y: (self.bounds.size.height - imgH ) / 2, width: imgH, height: imgH)
            }else{

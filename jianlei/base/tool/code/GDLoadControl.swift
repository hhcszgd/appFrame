...
public class GDLoadControl: GDBaseControl {
    public var loadHeight : CGFloat = 35
    ...
    public var pullingStr = "load_pullingStr"|?|
    public var loosenStr = "load_loosenStr"|?|
    public var loadingStr = "load_loadingStr"|?|
    public var successStr = "load_successStr"|?|
    public var failureStr = "load_failureStr"|?|
    public var networkErrorStr = "load_networkErrorStr"|?|
    public var nomoreStr = "load_nomoreStr"|?|
    /**
...
//            return
        ...
        case  GDDirection.bottom:
            ...
            if self.labelFrame == CGRect.zero {
                self.titleLabel.frame = CGRect(x: 0, y: (self.bounds.size.height - self.titleLabel.font.lineHeight ) / 2, width: self.bounds.size.width, height: self.titleLabel.font.lineHeight)
            }else{
                self.titleLabel.frame = self.labelFrame
            }
            if self.imageViewFrame == CGRect.zero {
//                self.imageView.frame = CGRect(x: 10, y: self.bounds.size.height * 0.2, width: self.bounds.size.height * 0.8, height: self.bounds.size.height * 0.8)
                ...
                let labelW = "load_nomoreStr"|?|.sizeSingleLine(font: titleLabel.font).width ?? 0
                let imgX : CGFloat = labelW > 0 ? self.bounds.width/2 - labelW/2 - imgH : self.bounds.width/2 - imgH * 3
                self.imageView.frame = CGRect(x: imgX, y: (self.bounds.size.height - imgH ) / 2, width: imgH, height: imgH)
            }else{
                self.imageView.frame = self.imageViewFrame
            }
//            if self.labelFrame == CGRect.zero {

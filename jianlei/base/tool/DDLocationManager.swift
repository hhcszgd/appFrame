...
class DDLocationManager: NSObject ,CLLocationManagerDelegate {
    ...
    func updateUserCurrentLoactionOnDisk(_ address:[AnyHashable:Any]?){
        if let countryName = address?["Country"] as? String{
            UserDefaults.standard.set(countryName, forKey: "DDCountryName")
        }
        if let countryCode = address?["CountryCode"] as? String{
            UserDefaults.standard.set(countryCode, forKey: "DDCountryCode")
        }
    }
    var userLocateCountry : (countryName:String,countryCode:String) {
        get{
            var para = (countryName:"",countryCode:"")
            if let countryName = UserDefaults.standard.value(forKey: "DDCountryName")  as? String{
                para.countryName = countryName
            }
            if let countryCode = UserDefaults.standard.value(forKey: "DDCountryCode") as? String{
...
//        }
    ...
    func placemarkFromLocation(location: CLLocation,completHandle:@escaping (CLPlacemark?)->())  {
        ...
        if #available(iOS 11.0, *) {
            let local = Locale.init(identifier: "language_identifier"|?|)

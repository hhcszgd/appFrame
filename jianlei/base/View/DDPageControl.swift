...
class DDPageControl: UIView {
    ...
    func _layoutSubviews() {
        ...
        if self.align == GDAutoAlignment.center {
            self.bounds = CGRect(x: 0, y: 0, width: totalWidth, height: itemH)
            self.center = CGPoint(x: (self.superview?.center.x ?? SCREENWIDTH/2), y: originFrame.origin.y + itemH / 2)
        }else if (self.align == GDAutoAlignment.left){
            self.frame = CGRect(x: padding, y: originFrame.origin.y, width: totalWidth, height: itemH)
        }else if (self.align == GDAutoAlignment.right){
            self.frame = CGRect(x: (self.superview?.bounds.size.width ?? SCREENWIDTH) - (totalWidth + padding), y: originFrame.origin.y, width: totalWidth, height: itemH)
        }

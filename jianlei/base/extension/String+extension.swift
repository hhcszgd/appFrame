...
//                let decrypted = try AES(key: key.bytes, blockMode: BlockMode.CBC(iv: iv.bytes), padding: .pkcs7).decrypt(targetData.bytes)
                let decrypted = try AES(key: key, iv: iv, padding: .pkcs7).decrypt(targetData.bytes)// AES(key: key, iv: iv, blockMode: CBC, padding: .pkcs7).decrypt(targetData.bytes)
...
//    }
    func mobileLawful() -> Bool {
        ...
        let xianggang = "^(|5|6|8|9)\\d{7}$"
        let xianggangpredicae = NSPredicate.init(format: "SELF MATCHES %@", xianggang)
        let result2: Bool = xianggangpredicae.evaluate(with: self)

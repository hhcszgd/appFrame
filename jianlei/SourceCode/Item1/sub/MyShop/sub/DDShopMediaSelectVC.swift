...
import UIKit
import Photos
class DDShopMediaSelectVC: DDInternalVC {
    deinit {
        mylog("image selecor destroyed")
    }
//    class ShopMediaModel: NSObject {
//        var asset : PHAsset?
//        var isUploadeed = false
//    }
    ...
    var uploadFailureAssets = [PHAsset]()
...
//            self.actionType = .uploaded
        }
    }

    func upload(index:Int , compated:@escaping ((Bool) -> ()))  {
        let model  = self.assets[index]
        DDPhotoManager.share.getMediaByPHAsset(asset: model) {[weak self] (image, imageInfo) in
            if let image = image{
...
//        a.targetSelectedCount = 30 - self.assets.count
        self.imagePicker = a
        a.completedHandle = {[weak self ] imageAssets in
            ...
            if let imageAssetsUnwrap = imageAssets , imageAssetsUnwrap.count > 0{
                ...
                self?.assets.append(contentsOf: imageAssetsUnwrap)
                self?.collection.reloadData()
            }else{
...
//        doneBtn.setImage(UIImage.init(named: "history"), for: UIControl.State.normal)
...
extension DDShopMediaSelectVC : DDItemDeleteActionDelegate {
...
extension DDShopMediaSelectVC : UICollectionViewDelegate ,UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if let itemUnwrap = item as? DDShopMediaSelectedItem {
            ...
            }else{itemUnwrap.deleteButton.isHidden = false }
...
class DDShopMediaSelectedItem : UICollectionViewCell {
    var model : PHAsset = PHAsset(){
        didSet{
            DDPhotoManager.share.getMediaByPHAsset(asset: model , targetSize: CGSize(width: 88, height: 88)) { (image , dict ) in
